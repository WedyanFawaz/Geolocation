...
import 'package:flutter_application_glocation/DistanceMeasure.dart';
...
class _MyHomePageState extends State<CurrentLoc> {
  ...
  Widget build(BuildContext context) {
    return  Scaffold(
      appBar: AppBar(
        title: Text('الموقع الحالي',style: TextStyle(fontWeight: FontWeight.bold,fontSize: 25)),
          ...
          children: [
            FloatingActionButton.extended(
              onPressed: () async {
                ...
                markers.add(Marker(
                    markerId: const MarkerId("الموقع الحالي"),
                ...
                child: FloatingActionButton.extended(
                    onPressed: () {
                      Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => DistanceMeasure()));
                    },
                    label: Text('عرض الطريق'),
                    icon: Icon(Icons.map),))
          ],
        ),
      ),
